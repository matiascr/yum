...
pub fn parse_node_tracks_scalar_styles_and_spans_test() {
  let assert Ok(plain) = yaml.parse_node("plain")
  let assert Ok(single) = yaml.parse_node("'value'")
  let assert Ok(double) = yaml.parse_node("\"value\"")
...
pub fn parse_node_tracks_block_sequence_style_and_span_test() {
  let assert Ok(document) = yaml.parse_node("- one\n- two\n")

  assert node.style(document) == node.BlockSequence
  assert node.span(document) == span(1, 1, 2, 6)
}

pub fn parse_node_tracks_block_mapping_style_and_span_test() {
  let assert Ok(document) = yaml.parse_node("one: 1\ntwo: 2\n")

  assert node.style(document) == node.BlockMapping
  assert node.span(document) == span(1, 1, 2, 7)
}

pub fn parse_node_tracks_flow_sequence_style_and_span_test() {
  let assert Ok(document) = yaml.parse_node("[one, two]")

  assert node.style(document) == node.FlowSequence
  assert node.span(document) == span(1, 1, 1, 11)
}

pub fn parse_node_tracks_flow_mapping_style_and_span_test() {
  let assert Ok(document) = yaml.parse_node("{one: 1, two: 2}")

  assert node.style(document) == node.FlowMapping
  assert node.span(document) == span(1, 1, 1, 17)
}

pub fn parse_node_tracks_block_scalar_styles_and_spans_test() {
  let assert Ok(literal_document) = yaml.parse_node("script: |\n  gleam test\n")
  let assert option.Some(literal) =
    node.get(literal_document, [node.Key("script")])
  let assert Ok(folded_document) =
    yaml.parse_node("script: >\n  gleam\n  test\n")
